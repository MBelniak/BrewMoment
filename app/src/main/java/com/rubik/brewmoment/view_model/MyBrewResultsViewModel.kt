...
class MyBrewResultsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun getAllResults(): LiveData<List<BrewResult>> {
        val result1 = BrewResult(2, 50, "Kostaryka La Candelilla",
            "Acidic", true, Date(SystemClock.elapsedRealtime()), true, isResultShared = false
        )
        val result2 = BrewResult(2, 50, "Kostaryka La Candelilla",
            "Acidic", false, Date(SystemClock.elapsedRealtime()), true, isResultShared = false
        )
